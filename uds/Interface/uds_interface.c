...
{
	if ((DTCSettingType == 0x01) || (DTCSettingType == 0x81))// ON
		uds_dtc_enable();
	else if ((DTCSettingType == 0x02) || (DTCSettingType == 0x82)) // OFF
		uds_dtc_disable();
}
void CommunicationType_Enable_Rx_Tx(communicationType)
{
	...
	{
	case 0x01: // 甯歌搴旂敤鎶ユ枃
		Signal_Control.receiver = 0;
		Signal_Control.send     = 0;
...
void Uds_CommunicationControl(uint8 controlType, uint8 communicationType)
{
	...
	case 0x01: // enable rx and disable tx
		...
		break;
	case 0x81: // enable rx and disable tx
		CommunicationType_Enable_Rx_Disable_Tx(communicationType);
		break;
...
// 	if(Speed_flag)
// 	{
// 		return Uds_E_CONDITIONSNOTCORRECT;
// 	}
		return E_OK; //	
}
...
//	{	
	//}
	if((resetType == 0x81) || (resetType == 0x01))
	{
		IWDG_Init(5,5);
	}
	return;
...
//   		securitySeed[0] = 0x01;
//        securitySeed[1] = 0x02;
		securitySeed[0] = current >> 8 & 0xFF;
		securitySeed[1] = current & 0xFF;
...
uint8 Uds_CompareKey(uint8 securityAccessType, uint8 *securitySeed, uint8 *securityKey)
{
    uint32 seed;
    uint32 key;
    uint32 validkey,Ivalidkey;

    seed= ((uint32)securitySeed[0]) << 8 |
    	  ((uint32)securitySeed[1] << 0);// |
    	  //((uint32)securitySeed[2] << 8) |
    	  //securitySeed[3];
    key = ((uint32)securityKey[0]) << 8 |
    	  ((uint32)securityKey[1] << 0);// |
    	  //((uint32)securityKey[2] << 8) |
    	 // securityKey[3];
    validkey = ComputeKey(securityAccessType, seed);
	Ivalidkey = validkey&0xFFFF;
    if (key == Ivalidkey)
    {
        return E_OK;
    }
...
uint8 vin[17]={0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF}; //F190 read & write
//uint8 Software_Reference_Number[10]={0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF};//F183 only read	
uint8 Software_Reference_Number[10]={0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0x42,0x00,0x00};//F183 only read	BSV1.02B  
uint8 EcuPartNumber[6]={0x43,0x00,0x14,0x31,0x16,0x00};//F187 {ASCLL,BCD,BCD,BCD,BCD,BCD}  only read   C 00 14 31 16 		件号
uint8 SystemSupplierID[6]={0x50,0x09,0x04,0x05,0x02,0x09};//F18A {ASCLL,BCD,BCD,BCD,BCD,BCD}  only read						供应商代码
uint8 EcuHardwareVersionNumber[6]={0x48,0x56,0x01,0x2E,0x00,0x04};//F191  {ASCLL,ASCLL,BCD,ASCLL,BCD,BCD}  only read HV1.04	硬件版本号
uint8 EcuApplicationSoftwareVersionNumber[6]={0x53,0x56,0x01,0x2E,0x02,0x00};//F1A0  {ASCLL,ASCLL,BCD,ASCLL,BCD,BCD}  only read  SV1.20	软件版本号
uint8 EcuCalibrationSoftwareNumber[6]={0x43,0x56,0x01,0x2E,0x00,0x00};//F1A1  {ASCLL,ASCLL,BCD,ASCLL,BCD,BCD}  only read  CV1.00		标定版本号
uint8 EcuNetReferenceNumber[6]={0x4E,0x56,0x01,0x2E,0x02,0x00};//F1A2  {ASCLL,ASCLL,BCD,ASCLL,BCD,BCD}  only read NV1.20				网络版本号
uint8 ECUManufactureDate[3]={0x19,0x07,0x11};//F18B  {BCD,BCD,BCD}  only read															制造日期
//uint8 SupplierECUSerialNumber[10]={0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF,0xFF};//F18C  {BCD,BCD,BCD}  only read
uint8 SupplierECUSerialNumber[10]={0x00,0x00,0x00,0x00,0x01,0x54,0x44,0x13,0x07,0x0B};//F18C  {BCD,BCD,BCD}  only read					
...
uint8 GeneralConfiguration[4]={0xFF,0xFF,0xFF,0xFF};//3700 read & write


void DID_VersionSet(void)
{
	u8 power = 0;
	u8 imagemode = 0;
	EcuApplicationSoftwareVersionNumber[2] = (*(Version+2))-'0';
	EcuApplicationSoftwareVersionNumber[4] = (*(Version+4))-'0';
	EcuApplicationSoftwareVersionNumber[5] = (*(Version+5))-'0';
	EcuNetReferenceNumber[2] = (*(Version+2))-'0';
	EcuNetReferenceNumber[4] = (*(Version+4))-'0';
	EcuNetReferenceNumber[5] = (*(Version+5))-'0';
	...
	imagemode = 2;
#endif
	EcuPartNumber[4]=0x15+power+imagemode;
	
#ifdef	RDS_ENABLE	//意大利 C00061025 改为 C00247452 
//EcuPartNumber[6]={0x43,0x00,0x06,0x10,0x25,0x00};
	EcuPartNumber[2] = 0x24;
	EcuPartNumber[3] = 0x74;
	EcuPartNumber[4] = 0x52;
#endif
	
#ifdef	FKY31	//京环件号	C00250123
//EcuPartNumber[6]={0x43,0x00,0x06,0x10,0x25,0x00};
	EcuPartNumber[2] = 0x25;
	EcuPartNumber[3] = 0x01;
	EcuPartNumber[4] = 0x23;
#endif
...
uint8 Uds_ReadDataByIdentifier(uint16 dataIdentifier, uint8 *dataRecord, uint8 *dataRecordSize)
{
	...
	temp = temp;
	tempresv = tempresv;
//	if(Speed_flag && uds.SessionLevel)
//	{
//		return Uds_E_CONDITIONSNOTCORRECT;
//	}
	...
	case 0xF190:
		for (i = 0; i < sizeof(vin); i++)
		{
			...
			if(TEST_DID_F190_Enable)
			{
				return Uds_E_RequestCorrectlyReceivedResponsePending;
			}
			else
			{
				dataRecord[index++] = TEST_DID_F190[i];
			}
		}
//		for (i = 0; i < sizeof(vin); i++)
//		{
...
//		}
	...
	case 0xF184:
		for (i = 0; i < 14; i++)
		{
			if(TEST_DID_F184_Enable)
			{
				return Uds_E_RequestCorrectlyReceivedResponsePending;
			}
			else
			{
				dataRecord[index++] = TEST_DID_F184[i];
			}
		}
//		for (i = 0; i < 14; i++)
//		{
//			FingerprintDataIdentifier[i] = TEST_DID_F184[i];
...
//		}
	...
	case 0xF1A3:
		...
		for (i = 0; i < 14; i++)
		{
			if(TEST_DID_F1A3_Enable)
			{
				return Uds_E_RequestCorrectlyReceivedResponsePending;
			}
			else
			{
				dataRecord[index++] = TEST_DID_F1A3[i];
			}
		}
//		for (i = 0; i < 14; i++)
//		{
//			CalibrationDataIdentifier[i] = TEST_DID_F1A3[i];
...
//		}
	...
	case 0x3700:
		for (i = 0; i < 4; i++)
		{
			if(TEST_DID_3700_Enable)
			{
				return Uds_E_RequestCorrectlyReceivedResponsePending;
			}
			else
			{
				dataRecord[index++] = TEST_DID_3700[i];
			}
		}
//		for (i = 0; i < 4; i++)
//		{
//			GeneralConfiguration[i] = TEST_DID_3700[i];
...
//	}
	...
	case 0xF184:
		...
		for (i = 0; i < 14; i++)
		{
			FingerprintDataIdentifier[i] = dataRecord[index++];
			TEST_DID_F184[i] = FingerprintDataIdentifier[i];
		}
		CAL_F184_CONF_WRITE();
		break;
	...
	case 0xF1A3:
		...
		for (i = 0; i < 14; i++)
		{
			...
			TEST_DID_F1A3[i]= CalibrationDataIdentifier[i];
		}
		CAL_F1A3_CONF_WRITE();
		break;
	...
	case 0x3700:
		...
		for (i = 0; i < 4; i++)
		{
			...
			TEST_DID_3700[i] = GeneralConfiguration[i];
		}
		CAL_3700_CONF_WRITE();
	//保存在EEPROM
