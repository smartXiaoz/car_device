...
static u8 busoff_cnt = 4;//50ms
static u8 busoff_tim = 0;//50ms
void BUSS_OFF(void)
{
	if(FAULT_Inf.BUSOFF)//Bus-Off
	{
		if(busoff_cnt>0)busoff_cnt--;
		else	
		{
			if(busoff_tim<6){busoff_tim++;busoff_cnt = 4;}
			else busoff_cnt = 9;
			CAN_Mode_Init(1,8,9,8,0);//初始化CAN
		}
	}
	else
	{
...
static u8 Can_Process(void)
{
	...
	if(busy == 0)	//空闲获取
	{
		...
		{
			if((SYS_Inf.STA == 2)&&(PowerNormal == 1))//系统启动，发送
			busy = CAN_CNT;
		}
	...
	if(CAR_Inf.ACC != 1) return 0;//ACC下电，不发送数据
	...
	if(ct1_cnt<99)ct1_cnt++;//1000MS时间同步
	else
	{
		ct1_cnt = 0;
		TX_RTCtime();
	}
	if(ct2_cnt<19)ct2_cnt++;//200MS音源同步
	else
	{
		ct2_cnt = 0;
		TX_Vsorce();
	}
	if(ct3_cnt<BLUE_Inf.time)ct3_cnt++;//1000MS蓝牙设备名同步
	else
	{
		ct3_cnt = 0;
		TX_BLUEinfo();
	}
	if(ct4_cnt<PHONE_Inf.time)ct4_cnt++;//通话信息同步
	else
	{
		ct4_cnt = 0;
		TX_PHONEinfo();
...
static u8 CarSet_Get3(u8 *Dat)
{
	CAR_Set.DDWS = (u8)((Dat[0]>>6)&0x03);		//疲劳驾驶开关
	return 0;
}  
