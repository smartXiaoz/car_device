            ...
            if (service->Subfunc != NULL)
            {
                if (sid == 0x2E)
                {
                  ...
                  //      return TRUE;
                    ...
                    if((uds.SecurityLevel == 1) &&(subFunc != 0xf10a))
					{
						 return TRUE;
					}
                }
...
void UdsInternal_ControlDTCSetting(PduIdType RxBufferID)
{
    ...
    //不支持所请求的子服务
    if ((ControlType!=0x01) && (ControlType != 0x02))
    {
...
void UdsInternal_CommunicationControl(PduIdType RxBufferID)
{
    ...
    if ((communicationType!=0x01) && (communicationType != 0x02) && (communicationType != 0x3))
    {
...
void UdsInternal_ECUReset(PduIdType RxBufferID) 
{
    ...
    //不支持所请求的子服务resetType
    if ((SubFunction!=0x01) &&(SubFunction!=0x03) && (SubFunction!=0x81) && (SubFunction!=0x83))
    {
