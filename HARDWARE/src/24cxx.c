...
u8 TEST_DID_F184[14],TEST_DID_F184_Enable;
...
void CAL_F184_CONF_WRITE(void) 
{
    TEST_DID_F184_Enable=1;
}
...
void Data_Save(void)
{
	...
	if(TEST_DID_F184_Enable==1)
	{
		for(i=0;i<14;i++)AT24CXX_WriteOneByte(EEPROM_ADD+17+14+4+i,(u8)(TEST_DID_F184[i]));
		TEST_DID_F184_Enable = 0;
	}
}
...
void Data_read(void)
{
	...
	for(i=0;i<14;i++)TEST_DID_F184[i] = AT24CXX_ReadOneByte(EEPROM_ADD+17+14+4+i);
}



#define Test_EEPadd	100
#define	Test_EEPlen	10
u8 eepwrite_cache[Test_EEPlen]={0};

void Test_EEPROMwrite(u8 len,u8 *dat)
{
	if(len>Test_EEPlen)return;
...
//		printf("it is new IC! \r\n");
		...
		AT24CXX_WriteOneByte(EEPROM_ADD+17+14,0XF4);
